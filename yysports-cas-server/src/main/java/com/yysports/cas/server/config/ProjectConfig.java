...
public class ProjectConfig extends WebMvcConfigurerAdapter {
	...
	public RedisTemplate<String, SystemAccountData> redisTemplateAccount(
		...
		RedisTemplate<String, SystemAccountData> template = new RedisTemplate<String, SystemAccountData>();
		template.setConnectionFactory(redisConnectionFactory);
		template.setKeySerializer(new StringRedisSerializer());
		ObjectMapper objectMapper = new ObjectMapper();
		objectMapper.configure(DeserializationFeature.ACCEPT_EMPTY_STRING_AS_NULL_OBJECT, true);
		objectMapper.configure(DeserializationFeature.FAIL_ON_IGNORED_PROPERTIES, false);
		Jackson2JsonRedisSerializer<SystemAccountData> jackson2JsonRedisSerializer = new Jackson2JsonRedisSerializer<SystemAccountData>(
