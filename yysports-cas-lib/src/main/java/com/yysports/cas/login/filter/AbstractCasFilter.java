...
import com.yysports.cas.login.utils.RestUtils;
...
public abstract class AbstractCasFilter extends OncePerRequestFilter
	...
	private static final List<String> staticFileExtensionList = new ArrayList<>(
	...
	@PostConstruct
	public void init() {
		if (!Objects.isNull(casProperties.getApExcludedUrls())) {
			excludedUris = excludedUris + "|"
					+ String.join("|", casProperties.getApExcludedUrls());
		}
		excludedUris = excludedUris + "|" + String.join("|", staticFileExtensionList);
	}
...
//		}
	...
	protected String getService(HttpServletRequest request) throws UnsupportedEncodingException {
		...
		return UriUtils.encodePath(url.toString(), Charsets.UTF_8.toString());
	}
